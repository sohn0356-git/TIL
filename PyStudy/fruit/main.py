...
#example:
classes = ['Banana', 'Avocado', 'Blueberry']
...
def main():
    ...
    for c in range(len(classes)):
        ...
        for i in range(nfiles):
            ...
            img = cv2.imread(path+infile, cv2.IMREAD_GRAYSCALE)
            ...
            fd = hog(img, orientations=9, pixels_per_cell=(32, 32),cells_per_block=(1,1))
            ...
            if(useList[3]):
                corners = corner_peaks(corner_harris(img), min_distance=5, threshold_rel=0.02)
    ...
    scores = cross_val_score(clf, DataSet, LabelSet, cv=3)
...
def edgeExtract(img, bins):
    ...
    #compute histograms
    lhist = np.histogram(fs,bins,range=(0,1))
    ahist = np.histogram(angs, bins,range=(-180,180))
